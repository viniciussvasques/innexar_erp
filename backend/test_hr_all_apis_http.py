...
# -*- coding: utf-8 -*-
"""
Teste completo de todas as APIs do módulo HR via HTTP
Não requer Django instalado - apenas requests
"""
import sys
import io
import requests
...
# Tentar descobrir o tenant do usuário
# Se não funcionar, usar o domínio padrão do tenant
TENANT_DOMAIN = None  # Será descoberto durante o login
EMAIL = "john@acme.com"
PASSWORD = "Test@123"
...
class HRAPITester:
    ...
    def test_request(self, method, url, data=None, expected_status=200, description=""):
        ...
        try:
            ...
            self.print_error(f"{description} - Erro de conexão. Servidor está rodando?")
            return None
    ...
    def login(self):
        ...
        # Lista de possíveis domínios de tenant para tentar
        tenant_domains = [
            "acme.localhost",
            "acme",
            "public.localhost",
            "testcompany.localhost",
            "localhost"
        ]
        
        # Primeiro tentar login público
        try:
            ...
            if response.status_code == 200:
                ...
                if self.token:
                    # Tentar descobrir o tenant do usuário
                    # Verificar resposta para tenant info
                    tenant_info = data.get('tenant') or data.get('tenant_domain')
                    if tenant_info:
                        if isinstance(tenant_info, dict):
                            # Se for um dicionário, extrair schema_name
                            schema_name = tenant_info.get('schema_name', 'acme')
                            tenant_domain = f"{schema_name}.localhost"
                        else:
                            tenant_domain = str(tenant_info)
                        
                        self.base_url = f"http://{tenant_domain}:8000"
                        self.print_info(f"Tenant descoberto: {tenant_domain}")
                    else:
                        # Tentar usar acme.localhost como padrão baseado no email
                        if 'acme' in EMAIL.lower():
                            self.base_url = "http://acme.localhost:8000"
                            self.print_info("Usando tenant padrão: acme.localhost")
                    
                    self.headers = {
                        'Authorization': f'Bearer {self.token}',
                        'Content-Type': 'application/json'
                    }
                    self.print_success("Login realizado com sucesso (público)")
        except:
            pass
        
        # Se não conseguiu token público, tentar nos tenants
        if not self.token:
            self.print_info("Tentando login nos tenants...")
            for domain in tenant_domains:
                try:
                    tenant_url = f"http://{domain}:8000"
                    tenant_login = f"{tenant_url}/api/v1/auth/login/"
                    response = requests.post(tenant_login, json=login_data, timeout=5)
                    if response.status_code == 200:
                        data = response.json()
                        self.token = data.get('access') or data.get('token')
                        if self.token:
                            ...
                            self.print_success(f"Login realizado com sucesso no tenant: {domain}")
                            return True
                except requests.exceptions.ConnectionError:
                    continue
                except Exception as e:
        ...
        if self.token:
            ...
            self.print_error("Falha no login em todos os domínios tentados")
            self.print_info("Verifique se o servidor está rodando e se as credenciais estão corretas")
            return False
